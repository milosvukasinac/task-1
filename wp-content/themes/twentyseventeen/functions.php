...
function products_admin_menu() {
	global $product_page;
	add_menu_page( __( 'Products', 'sports-bench' ), __( 'Products', 'sports-bench' ), 'edit_posts', 'add_data', 'products_handler', 'dashicons-groups', 6);
}
...
function products_handler(){
	global $wpdb;
	echo '<form class="myForm" method="POST" action="?page=add_data" enctype="multipart/form-data">
	...
	$table_name = 'products';
	$default_row = $wpdb->get_row( "SELECT * FROM $table_name ORDER BY id DESC LIMIT 1" );
	if ( $default_row != null ) {
	$id = $default_row->id + 1;
	} else {
	$id = 1;
	...
	$post_id = $wpdb->insert( $table_name, $item );
